...
//based on JSONEncoder.swift from vapor/vapor
// https://github.com/vapor/vapor/blob/main/Sources/Vapor/Utilities/URI.swift
import CURLParser
import Foundation
...
extension URI {
    public func decode<D:Decodable>(_ decodable:D.Type) throws -> D {
        let query = URLQueryContainer.init(url: self)
        return try query.decode(D.self)
    }
}
